...
HttpConnection::HttpConnection(boost::asio::io_service& io_service, RequestHandler& handler, bool updater_respond) :
...
void HttpConnection::start()
{
	socket_.async_read_some(boost::asio::buffer(buffer_), strand_.wrap(
		boost::bind(&HttpConnection::handle_read, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred)));
}
	...
	if(!e)
	{
		...
		if(result)
		{
			...
			boost::asio::async_write(socket_, reply_.to_buffers(), strand_.wrap(
				boost::bind(&HttpConnection::handle_write, shared_from_this(), boost::asio::placeholders::error)));
		}
		else if(!result)
		{
...
void HttpConnection::handle_write(const boost::system::error_code& e)
{
	if(!e)
	{
		std::string content = reply_.content.c_str();
		size_t pos = 0;
		while((pos = content.find('\n', pos)) != std::string::npos)
			content.erase(pos, 1);
		pos = 0;
		while((pos = content.find('\r', pos)) != std::string::npos)	// many carriage returns are in the installConfig
			content.erase(pos, 1);
...
std::string HttpConnection::getLocalIp()
{
...
std::string HttpConnection::getRemoteIp()
{
